...
def remove_all_cards(player: Player) -> bool:
    ...
    try:
        ...
        db.session.rollback()
        print(f"An error occurred while removing all cards from player {player.id}: {e}")
        return False
...
def remove_card_from_player(player: Player, card_id: int) -> bool:
    ...
    try:
        ...
        db.session.rollback()
        print(f"An error occurred while removing card {card_id} from player {player.id}: {e}")
        return False
